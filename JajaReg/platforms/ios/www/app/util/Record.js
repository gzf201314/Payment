Ext.define("util.Record", {
    ...
    * 用来判断是否为管理员注册
    */
    isRegAdmin: function (RegUserID) {
        var isAdmin = false;
        Record.getList('RegAdmin', RegUserID, function (jsonStr) {
            if (jsonStr == "001") {
                isAdmin = true;
            }
            else {
    ...
    */
    ...
    Pending: function (callback1, callback2) {
        this.getList('Pending', { // 待处理
            RegBeginTime: '',
            RegEndTime: '',
            HotelName: '',
            HotelAddress: '',
    ...
    */
    loadMenu: function () {
        //var data = this.menumodel();
        // console.log(data);
        this.store = Ext.create('Ext.data.Store', {
            //define the stores fields
             fields: ['RegMenuID', 'Message', 'Remark', 'badgeText', 'Count'],
    ...
    query: function (data, fn) {
        var me = this,
            ...
            sender = RecordTitle.substring(0, 1);
        switch (sender) {
            case "试":
                me.getList('TrialList', data, fn);
                break;
            case "正":
                me.getList('ApplyList', data, fn);
                break;
            case "申":
                me.getList('ApplyList', data, fn);
                break;
    ...
    // 明细
    detail: function (data, fn) {
        var me = this,
            RecordTitle = localStorage.getItem('RecordTitle'),
            sender = RecordTitle.substring(0, 1);
        switch (sender) {
            case "试":
                me.getList('TrialDetail', data, fn);
                break;
            case "正":
                me.getList('ApplyDetail', data, fn);
                break;
            case "申":
                me.getList('ApplyDetail', data, fn);
                break;
    ...
    // 获取数据列表
    getList: function (key, value, callback, errorCallBack) {
    ...
    // 销毁
    destroy: function () {
        if (Ext.getCmp(this.back)) {
            console.log(Ext.getCmp(this.back));
            Ext.getCmp(this.back).destroy();
        }

        if (Ext.getCmp(this.search)) {
            Ext.getCmp(this.search).destroy();
        }

        if (Ext.getCmp(this.add)) {
            Ext.getCmp(this.add).destroy();
        }
    },
    // 获取资料明细
    getDetail: function (data) {
        /* 
        TrialDetail、ApplyDetail
        */
        var RegID = data, me = this;
        Progress.start("正在加载,请稍后...");
        setTimeout(function () {
            me.detail(data, function (objs) {
                console.log(JSON.parse(objs));
                Progress.close();
                //notification.alert(JSON.parse(objs).AskTime);
                me.showDetailView(JSON.parse(objs));
            });
        }, 500);
    },
    // 正式信息
    initWithFormalMsgView: function () {
        Progress.start("正在加载,请稍后...");
        var me = this;
        setTimeout(function () {
            me.getList('ApplyList', {
                RegBeginTime: '',
                RegEndTime: '',
                HotelName: '',
                HotelAddress: '',
                ...
                var dataSet = JSON.parse(jsonStr);
                // console.log(dataSet);
                me.showRecordView("正式", me.getTrialListConfiguration('formal', dataSet));
                Progress.close();
            });
        }, 500);
    },
    // 申请信息
    initWithApplyMsgView: function () {
        Progress.start("正在加载,请稍后...");
        var me = this;
        setTimeout(function () {
            me.getList('ApplyList', {
                ...
                var dataSet = JSON.parse(jsonStr);
                console.log(dataSet);
                me.showRecordView("申请", me.getTrialListConfiguration('apply', dataSet));
                Progress.close();
            });
        }, 500);
    },
    ...
    * 授权用户记录表
    */
    getUserListConfiguration: function (data) {
        var listview = Ext.create('Ext.dataview.List', {
            ...
            itemTpl: '<div id="{UserID}" style="width:100%;position: absolute;padding: 8px;"><div class="content" style="width:80%;"><div class="name">{CompanyName}</div> <div class ="affiliation">{Address}</div></div><div class="date" type="date" style="float: right;height: 41px;line-height: 41px;width: 20%; margin-top: -38px;margin-right:6px; text-align: center;">{UserName}</div></div>',
            ...
            items: [{
                ...
                items: [{
                    ...
                    draggable: false,
                    height: 47,
                    placeHolder: '请输入公司名称/地址/姓名',
                    listeners: {
                        scope: this,
                        clearicontap: this.onSearchUserClearIconTap,
                        keyup: this.onSearchUserKeyUp
                    }
                }, {
                    xtype: 'button',
                    ...
                    listeners: {
                        scope: this,
                        tap: this.onCancelSearchUser
                    }
                }]
            }]
        });

        listview.setStore(this.getUserStore(data)).setData(data);
    ...
    */
    getProxyOfficialListConfiguration: function (data) {
        var listview = Ext.create('Ext.dataview.List', {
            ...
            style: 'width:100%;height:100%;',
            itemTpl: '<div style="width:100%;position: absolute;padding: 9px 10px 10px 10px;"><div class="content" style="width:60%;"><div class="name">{RegName}</div> <div class ="affiliation">{RegUserID}</div></div><div class="date" type="date" style="float: right;height: 38px;line-height: 38px;width: 40%; margin-top: -38px;margin-right:6px; text-align: center;">{OffTime}</div></div>',
            //store: this.getOfficialStore(data),
            useSimpleItems: true,
            //grouped: true,
            emptyText: '<div style="margin-bottom:200px;text-align: center;color:rgb(173,170,173);">没有查询到相关数据</div>',
    ...
    */
    getProxyListConfiguration: function (data) {
        var listview = Ext.create('Ext.dataview.List', {
            ...
            style: 'width:100%;height:100%;',
            itemTpl: '<div style="width:100%;position: absolute;padding: 9px 10px 10px 10px;"><div class="content" style="width:60%;"><div class="name">{RegName}</div> <div class ="affiliation">{RegUserID}</div></div><div class="date" type="date" style="float: right;height: 41px;line-height: 22px;width: 40%; margin-top: -41px;margin-right:6px; text-align: center;"><div>{RegTime}</div><div style="color:rgb(247,77,24);">{TryDate}</div></div></div>',
