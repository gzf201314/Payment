...
#pragma mark --- 状态栏与导航栏合并的解决方法
- (void)customizeWithView
{
    // 导航与状态栏合并的问题
    ...
    if (currentVersion >= 7) {
        ...
        NSScanner* scanner = [NSScanner scannerWithString:@"#178D5"];
        [scanner setScanLocation:1];
        [scanner scanHexInt:&rgbValue];
        
       // [[UIApplication sharedApplication] setStatusBarStyle:UIStatusBarStyleBlackOpaque];
        [self prefersStatusBarHidden];
        NSLog(@"status:%d",[self prefersStatusBarHidden]);
...
- (void)webViewDidFinishLoad:(UIWebView*)theWebView
{
    ...
    theWebView.backgroundColor = KBackgroundColor;
    
    return [super webViewDidFinishLoad:theWebView];
   // [MBProgressHUD showMessag:@"Loading..." toView:theWebView];
   // [MBProgressHUD hideAllHUDsForView:theWebView animated:YES];
}
